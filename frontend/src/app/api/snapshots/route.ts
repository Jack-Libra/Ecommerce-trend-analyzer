...
export async function GET(req: NextRequest) {
  ...
  const normalized: ProductSnapshot[] = (data ?? []).map((raw: any) => ({
    ...
    created_at: 
    raw.created_at ? new Date(raw.created_at).toISOString() : undefined,
    captured_at: 
    raw.captured_at ? new Date(raw.captured_at).toISOString() : undefined,
  }));
